				...
				<div class="container-fluid">
					...
					<div class="card">
						...
						<div class="card-header">
							...
							<form
								action="${pageContext.request.contextPath}/GoodsServlet?method=findAllGoodsByNameOrTypeExamine&firstpage=0"
								method="post">
								...
								<tbody>
									...
									<c:forEach items="${ goodlist }" var="gl" varStatus="i">
										<tr>
											...
											<c:if test="${ gl.goodstate == 0 }">
												<th>未上架</th>
											</c:if>
											<c:if test="${ gl.goodstate == 1 }">
												<th>已上架</th>
											</c:if>
											<c:if test="${ gl.goodstate == 2 }">
												<th>下架</th>
											</c:if>
											...
											<th>
											<a class="active"
												href="${pageContext.request.contextPath}/GoodsServlet?method=approvedGood&goodname=${ gl.goodname }&goodtypecode=${ gl.goodType.goodtypecode }&goodcode=${gl.goodcode}&firstpage=${firstpage}">审核<br>通过</a>
												
											</th>
											<th>
											<a class="active"
												href="${pageContext.request.contextPath}/GoodsServlet?method=noapprovedGood&goodname=${ gl.goodname }&goodtypecode=${ gl.goodType.goodtypecode }&goodcode=${gl.goodcode}&firstpage=${firstpage}">审核<br>不通过</a>
									...
									<c:if test="${ not empty goodlist }">
										...
										<c:if test="${ 0!=firstpage }">
											<li class="page-item"><a
												href="${pageContext.request.contextPath}/GoodsServlet?method=findAllGoodsByNameOrTypeExamine&goodname=${ goodname }&goodtypecode=${ goodtypecode }&firstpage=${firstpage-1}"
										...
										<c:forEach begin="1" end="${maxPageNum}" step="1" var="i">
											...
											<c:if test="${ i!= firstpage }">
												<li class="page-item"><a
													href="${pageContext.request.contextPath}/GoodsServlet?method=findAllGoodsByNameOrTypeExamine&goodname=${ goodname }&goodtypecode=${ goodtypecode }&firstpage=${ i }"
